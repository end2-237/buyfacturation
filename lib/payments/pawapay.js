...
// On nettoie l'URL (espaces / slash final) : une variable d'env mal collée
// (« https://api.pawapay.io ») cassait la construction de l'URL.
...
export const PawaPayProvider = {
  ...
  async initierPaiement({ depositId, montant, devise, numero, operateur, reference }) {
    ...
    const body = {
      ...
      // PawaPay n'accepte que des caractères alphanumériques et des espaces.
      customerMessage: String(reference || "BUYTICLE")
        .replace(/[^a-zA-Z0-9 ]/g, " ")
        .replace(/\s+/g, " ")
        .trim()
        .slice(0, 22) || "BUYTICLE",
