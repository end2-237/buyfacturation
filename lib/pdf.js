...
// ── Recu de versement ─────────────────────────────────────────────────────────
// L'argent va dans l'autre sens : la plateforme verse au beneficiaire. Rien
// n'est du, personne n'a a payer en lisant ce papier — il atteste, il ne
// reclame pas. D'ou un vocabulaire different de bout en bout : « verse a »
// plutot que « facture a », « montant verse » plutot que « net a payer », et
// aucune mention de TVA ni d'echeance.
function RecuDocument({ inv }) {
  ...
  return (
    <Document>
      <Page size="A4" style={s.page}>
        ...
        <Text style={[s.bold, { fontSize: 18, color: accent, marginBottom: 4 }]}>REÇU DE VERSEMENT</Text>
        <View style={{ backgroundColor: "#0F7B4F", alignSelf: "flex-start", padding: "3px 10px", marginBottom: 10 }}>
          <Text style={{ color: C.white, fontFamily: "Helvetica-Bold", fontSize: 7.5 }}>VERSEMENT EFFECTUÉ</Text>
        </View>

        {inv.platform ? (
          <Text style={{ color: C.gray, marginBottom: 14 }}>
            Plateforme émettrice : <Text style={s.bold}>{inv.platform}</Text>
            {inv.platform_url ? <>{"  "}·{"  "}<Text style={{ color: C.darkOrange }}>{inv.platform_url}</Text></> : null}
          </Text>
        ) : null}

        <View style={[s.row, { marginBottom: 14 }]}>
          <View style={{ flex: 1, paddingRight: 10 }}>
            <Text style={[s.label, { color: accent, marginBottom: 4 }]}>VERSÉ À</Text>
            <Text style={s.bold}>{inv.client_name}</Text>
            {inv.client_address && <Text>{inv.client_address}</Text>}
            {inv.client_phone && <Text>{inv.client_phone}</Text>}
            {inv.client_email && <Text>{inv.client_email}</Text>}
          ...
          <View style={s.metaBox}>
            <MetaRow label="N° de reçu" value={inv.number} bold />
            <MetaRow label="Date du versement" value={fmtDate(inv.date)} />
            {inv.statut ? <MetaRow label="Moyen" value={inv.statut} /> : null}
            {/* La reference de transaction est le seul element verifiable par
                les deux parties aupres de l'operateur : elle merite sa ligne. */}
            {inv.reference ? <MetaRow label="Réf. transaction" value={inv.reference} /> : null}
          </View>
        </View>

        <View style={[s.row, { backgroundColor: accent }]}>
          <Text style={[s.th, { flex: 4 }]}>OBJET DU VERSEMENT</Text>
          <Text style={[s.th, { flex: 2, textAlign: "right" }]}>MONTANT</Text>
        </View>
        {(items.length ? items : [{ description: "Versement", quantity: 1, price: total }]).map((it, i) => (
          <View key={i} style={s.row}>
        ...
        <View style={[s.row, { justifyContent: "flex-end", marginTop: 4, marginBottom: 8 }]}>
          <View style={{ width: 240 }}>
            <View style={[s.totalRow, { backgroundColor: accent }]}>
              <Text style={[s.totalTd, { flex: 1 }]}>MONTANT VERSÉ</Text>
              <Text style={s.totalTd}>{fmt(total)}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={[s.noteBox, { borderLeftColor: accent }]}>
          <Text>
            Reçu arrêté à la somme de{" "}
            <Text style={[s.bold, { color: accent }]}>{numberToWords(Math.round(total))} francs CFA</Text>,
            versés par {v.name || "la plateforme"} au bénéficiaire ci-dessus.
          </Text>
        </View>

        {/* Le recours, ecrit sur la piece elle-meme : un beneficiaire qui n'a
            rien vu arriver ne doit pas avoir a chercher qui prevenir. */}
        {inv.notice ? (
          <View style={{ borderLeft: `3px solid ${C.gray}`, backgroundColor: C.bgPale, padding: "8px 10px", marginBottom: 10 }}>
            <Text style={{ color: C.gray }}>{inv.notice}</Text>
          </View>
        ) : null}
...
export async function generatePdf(invoice) {
  ...
  const Doc = inv.type === "abonnement" ? AboDocument
            : inv.type === "recu"       ? RecuDocument
            : StdDocument;
