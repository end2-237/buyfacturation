...
//
// Les valeurs historiques restent le defaut, cle par cle : un document sans
// `seller`, ou avec un `seller` partiel, s'imprime exactement comme avant.
const VENDEUR_DEFAUT = {
  name: "BUYTICLE ETS",
...
function vendeur(inv) {
  const s = inv?.seller && typeof inv.seller === "object" ? inv.seller : {};
  const merged = { ...VENDEUR_DEFAUT };
  for (const [k, v] of Object.entries(s)) {
    // Une chaine vide est une case laissee vide dans le formulaire, pas un
    // effacement volontaire : on garde le defaut plutot qu'un trou dans l'en-tete.
    if (v !== null && v !== undefined && String(v).trim() !== "") merged[k] = v;
  }
  // Le NIU reste surchargeable par la colonne dediee, qui lui est anterieure.
  if (inv?.niu) merged.niu = inv.niu;
  return merged;
}
...
function CompanyHeader({ inv }) {
  ...
  return (
    <View style={[s.row, { justifyContent: "space-between", marginBottom: 4 }]}>
      ...
      {/* Le logo est facultatif et volontairement tolerant : une image
          injoignable ne doit pas empecher le document de sortir. */}
      {v.logo_url ? (
        <Image src={v.logo_url} style={{ maxWidth: 110, maxHeight: 55, objectFit: "contain" }} />
      ) : null}
    </View>
  );
}
